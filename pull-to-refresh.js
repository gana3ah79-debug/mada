(()=>{
  const THRESHOLD=78,MAX_PULL=110;
  ...
  function touchMove(e){if(isBlocked()||getScrollTop()>2||!startY)return;const y=e.touches[0]?.clientY||startY;lastY=y;const delta=y-startY;if(delta<=0)return;pulling=true;const eased=Math.min(MAX_PULL,delta*.62);show(eased);if(eased>8)e.preventDefault()}
  function touchEnd(){if(!pulling){startY=lastY=0;return}const delta=lastY-startY,start=startY;startY=lastY=0;if(delta*.62>=THRESHOLD){refresh()}else hide()}
