...
(function(){
  ...
  async function getClient(){
    return window.sb || (window.supabase?.createClient&&window.MADA_SUPABASE_URL&&window.MADA_SUPABASE_KEY
  ...
  async function verifyAndRestore(){
    ...
    try{
      ...
      return session||null;
    }catch(e){console.warn('Mada auth stability',e);return null}
  ...
  function init(){
    ...
    if(c?.auth?.onAuthStateChange){
      c.auth.onAuthStateChange((event,session)=>{
        if(session){window.user=session.user;const a=document.getElementById('auth'),app=document.getElementById('app');if(a)a.hidden=true;if(app)app.hidden=false;return;}
        if(event==='SIGNED_OUT'){window.user=null;const a=document.getElementById('auth'),app=document.getElementById('app');if(a)a.hidden=false;if(app)app.hidden=true;return;}
