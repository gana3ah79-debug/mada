-- Applied to the Mada Supabase production project.
-- Keeps the repository in sync with the production schema changes.
...
create policy if not exists post_shares_read on public.post_shares for select to authenticated using (user_id=auth.uid() or target_user_id=auth.uid() or public.is_admin());
create policy if not exists post_shares_insert on public.post_shares for insert to authenticated with check (user_id=auth.uid());
...
-- Social notifications are generated by a SECURITY DEFINER trigger and are not callable as an RPC.
