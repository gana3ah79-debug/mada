...
async function hydrate(){
 ...
 try{
  const u=me();
  const posts=await s.from('posts').select('id,author_id').in('id',ids); const list=posts.data||[];
  const pm=new Map(list.map(x=>[String(x.id),x])); const authorIds=[...new Set(list.map(x=>x.author_id).filter(Boolean))];
  const [views,saves,following,allF]=await Promise.all([
   s.from('post_views').select('post_id').in('post_id',ids),
   s.from('post_saves').select('post_id,user_id').in('post_id',ids),
   u&&authorIds.length?s.from('follows').select('following_id').eq('follower_id',u.id).in('following_id',authorIds):Promise.resolve({data:[]}),
   authorIds.length?s.from('follows').select('following_id').in('following_id',authorIds):Promise.resolve({data:[]})
  ]);
  ...
  cards.forEach(card=>{
   const id=String(card.dataset.reel||''),p=pm.get(id),author=p?.author_id;if(!author)return; card.dataset.madaAuthor=author;
   const info=card.querySelector('.mr-info');if(!info)return;
   ...
   if(!fb&&u&&author!==u.id){fb=document.createElement('button');fb.className='mrs-follow';fb.type='button';row.appendChild(fb);fb.onclick=e=>{e.preventDefault();e.stopPropagation();toggleFollow(author,fb)}}
