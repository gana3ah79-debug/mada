...
(function(){
  ...
  const getPost=id=>window.feedPosts?.get(id);
  ...
  function decoratePost(post){
    ...
    ['mousedown','touchstart'].forEach(ev=>like.addEventListener(ev,e=>{
  ...
  function refreshPost(id){const p=document.querySelector(`article.post[data-post-id="${CSS.escape(id)}"]`);if(p){p.querySelector('.post-actions')?.removeAttribute('data-social-ready');decoratePost(p);}}
  ...
  async function setReaction(postId,type){
    ...
    try{
      ...
      // Re-read this post's reactions so counters and emoji are always database-backed.
  ...
  function openStory(id,all){
    const index=Math.max(0,all.findIndex(x=>x.id===id)); const stories=all.filter(x=>new Date(x.expires_at)>new Date());if(!stories.length)return;
    const start=stories.findIndex(x=>x.id===id);let i=start>=0?start:index;
  ...
  // app.js already has sharePost; this wrapper guarantees a clipboard fallback when native share is unavailable.
  window.madaNativeShare=async function(postId){
    const p=getPost(postId),url=`${location.origin}${location.pathname}#post-${encodeURIComponent(postId)}`,text=p?.body||'شاهد هذا المنشور على Mada';
    try{if(navigator.share){await navigator.share({title:'Mada',text,url});}else if(navigator.clipboard){await navigator.clipboard.writeText(url);alert('تم نسخ رابط المنشور');}else{prompt('انسخ رابط المنشور:',url);}if(window.sb&&window.user)await sb.from('post_shares').insert({post_id:postId,user_id:user.id}).catch(()=>{});}catch(e){if(e?.name!=='AbortError')console.warn(e);}
  };
  document.addEventListener('click',e=>{const b=e.target.closest?.('.share');if(b){e.preventDefault();e.stopImmediatePropagation();window.madaNativeShare(b.dataset.id);}},true);
})();