...
(function(){
  ...
  function decoratePost(post){
    ...
    const count=parseCount(like.textContent);let stats=post.querySelector('.post-stats');if(!stats){stats=document.createElement('div');stats.className='post-stats';actions.before(stats);}stats.innerHTML=`<span class="reaction-summary"><span class="summary-icons">👍</span><span class="reaction-total">${count}</span></span><span class="comment-summary">${comment?parseCount(comment.textContent):0} تعليق</span>`;
  ...
  async function setReaction(postId,type){
    ...
    try{if(type==='none'){const{error}=await sb.from('post_likes').delete().eq('post_id',postId).eq('user_id',user.id);if(error)throw error;if(btn){btn.dataset.liked='false';delete btn.dataset.reaction;}}else{const{error}=await sb.from('post_likes').upsert({post_id:postId,user_id:user.id,reaction_type:type},{onConflict:'post_id,user_id'});if(error)throw error;if(btn){btn.dataset.liked='true';btn.dataset.reaction=type;}}
      const{data,error}=await sb.from('post_likes').select('user_id,reaction_type').eq('post_id',postId);if(error)throw error;const likes=data||[];const mine=likes.find(x=>x.user_id===user.id)?.reaction_type||null;if(btn){btn.dataset.liked=String(!!mine);if(mine)btn.dataset.reaction=mine;else delete btn.dataset.reaction;btn.innerHTML=`<span class="reaction-icon">${mine?reactionMap[mine]:'👍'}</span><span>${mine?reactionNames[mine]:'إعجاب'}</span>`;}const post=document.querySelector(`article.post[data-post-id="${CSS.escape(postId)}"]`);const stats=post?.querySelector('.post-stats');if(stats){const groups={};likes.forEach(x=>{groups[x.reaction_type]=(groups[x.reaction_type]||0)+1;});const top=Object.entries(groups).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([k])=>reactionMap[k]).join('');stats.querySelector('.summary-icons').textContent=top||'👍';stats.querySelector('.reaction-total').textContent=likes.length;}}
  ...
  const observer=new MutationObserver(()=>document.querySelectorAll('#feed article.post').forEach(decoratePost));observer.observe(document.getElementById('feed')||document.body,{childList:true,subtree:true});
  document.addEventListener('DOMContentLoaded',()=>{setTimeout(loadStories,1200);setTimeout(()=>{refreshSocialBadges();startSocialRealtime()},1800);});
})();
