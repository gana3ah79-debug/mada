/* Mada message/search bridge: keep one stable entry point for the header buttons. */
(function(){
  ...
  function renderHome(){
    const c=content();if(!c)return;const a=recent();
  ...
  async function search(q){
    ...
    try{
      ...
      c.querySelectorAll('.result-row').forEach((r,i)=>r.onclick=()=>openUser(data[i]));
  ...
  async function openUser(p){
    saveRecent(p);
    try{
      if(typeof window.getOrCreateConversation!=='function'||typeof window.openConversation!=='function'){
        /* Messenger-modern is the canonical UI; open it and let its own search/new-chat flow handle the user. */
        if(typeof window.madaMessenger==='function'){window.madaMessenger();return}
        throw new Error('نظام الرسائل غير جاهز');
      }
  ...
  function bind(){
    ...
    if(msg&&msg.dataset.messagesBound!=='1'){
      msg.dataset.messagesBound='1';msg.type='button';
      msg.onclick=e=>{e.preventDefault();e.stopPropagation();
        if(typeof window.madaMessenger==='function')window.madaMessenger();
        else window.openMessages?.();
      };
    }
    const searchBtn=$('searchBtn');
    if(searchBtn&&searchBtn.dataset.modernSearchBound!=='1'){
      searchBtn.dataset.modernSearchBound='1';searchBtn.type='button';searchBtn.onclick=e=>{e.preventDefault();e.stopPropagation();openSearch()};
    }
    const close=$('closeModal');
    if(close&&!close.dataset.closeBound){close.dataset.closeBound='1';close.onclick=()=>{$('modal').hidden=true;$('modal').classList.remove('modern-search-modal')}}
  }
