...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b) {
        ...
        setContentView(web);
        web.loadUrl(LOCAL_URL + "?apk=full&v=14");
    }
    ...
    void syncSession() {
        String js = "(async()=>{try{let s=window.MADA_SUPABASE_CLIENT;if(!s)return null;let r=await s.auth.getSession();let x=r.data&&r.data.session;return x?JSON.stringify({access_token:x.access_token,refresh_token:x.refresh_token,user_id:x.user.id}):null}catch(e){return null}})()";
    ...
    void openPendingChat() {
        ...
        String js = "(()=>{try{if(window.MadaMessenger&&window.MadaMessenger.openFriend){window.MadaMessenger.openFriend(" + id + ");return true}return false}catch(e){return false}})()";
    ...
    void grantPendingMic(boolean granted) {
        ...
        if (granted && r != null) { try { r.grant(new String[]{PermissionRequest.RESOURCE_AUDIO_CAPTURE}); } catch (Exception ignored) {} }
        else if (r != null) { try { r.deny(); } catch (Exception ignored) {} }
