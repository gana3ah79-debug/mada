// Mada auth controller: one shared Supabase session for the whole app.
(function(){
  const $=id=>document.getElementById(id);
  const btn=$('loginBtn');
  if(!btn) return;
  const setMsg=t=>{const m=$('authMsg');if(m)m.textContent=t||''};
  const client=()=>window.MADA_SUPABASE_CLIENT||sb;
  async function login(){
    ...
    try{
      ...
      if(!data?.session||!data?.user){setMsg('تم الدخول لكن لم يتم إنشاء جلسة.');return;}
      setMsg('تم تسجيل الدخول، جاري فتح Mada…');
      if(typeof start==='function') await start(); else location.reload();
  ...
  async function register(){
    ...
    try{
      ...
      if(data?.session){setMsg('تم إنشاء الحساب، جاري فتح Mada…');if(typeof start==='function')await start();}
      else setMsg('تم إنشاء الحساب. أكد بريدك الإلكتروني ثم اضغط دخول.');
    }catch(e){setMsg(e?.message||'حدث خطأ أثناء إنشاء الحساب.');}
  }
  btn.onclick=login;btn.textContent='دخول';
  if(!$('registerBtn')){const b=document.createElement('button');b.id='registerBtn';b.type='button';b.textContent='إنشاء حساب جديد';b.style.cssText='display:block;width:100%;margin:8px 0;padding:13px;border:0;border-radius:13px;font-size:16px;font-weight:700;';btn.parentNode.insertBefore(b,btn.nextSibling);b.onclick=register;}
})();
