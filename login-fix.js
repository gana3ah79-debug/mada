...
(function(){
  ...
  async function login(){
    ...
    try{
      ...
      // Do NOT reload. Reuse the app's real startup path so the same Supabase client/session
      // is used to switch the UI and load profile/feed.
      setMsg('تم تسجيل الدخول، جاري فتح Mada…');
      if(typeof start==='function') await start();
      else { $('auth').hidden=true; $('app').hidden=false; }
