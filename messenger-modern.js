...
(function(){
  ...
  async function loadConversations(){
    ...
    try{
      ...
      const {data:msgs}=await sb.from('messages').select('id,conversation_id,sender_id,body,created_at,message_type,media_url,file_name,deleted_at').in('conversation_id',ids).order('created_at',{ascending:false}).limit(100);
      ...
      list.innerHTML=rows.map(x=>'<button class="mm-row" type="button" data-cid="'+esc(x.cid)+'" data-uid="'+esc(x.p.id)+'"><span class="mm-avatar">'+avatar(x.p)+'</span><span class="mm-info"><b>'+esc(x.p.display_name||'مستخدم Mada')+'</b><small>@'+esc(x.p.username||'')+'</small><em>'+esc(preview(x.m)||'ابدأ المحادثة')+'</em></span><span class="mm-time">'+time(x.m?.created_at)+'</span></button>').join('');
  ...
  function preview(m){if(!m)return '';if(m.deleted_at)return 'تم حذف هذه الرسالة';if(m.message_type==='image')return '🖼️ صورة';if(m.message_type==='video')return '🎬 فيديو';if(m.message_type==='gif')return 'GIF';if(m.message_type==='audio')return '🎤 تسجيل صوتي';if(m.message_type==='file')return '📎 ملف';return m.body||''}
  ...
  async function loadMessages(){
    ...
    const {data,error}=await sb.from('messages').select('id,conversation_id,sender_id,body,created_at,read_at,message_type,media_url,file_name,file_size,reply_to_id,deleted_at,shared_from_id').eq('conversation_id',current.cid).order('created_at',{ascending:true}).limit(300);if(error){out.innerHTML='<div class="mm-empty">تعذر تحميل المحادثة</div>';return}
    ...
    if(window.madaMessengerAdvanced?.afterLoad)window.madaMessengerAdvanced.afterLoad(data||[]);
  }
  function messageHtml(m){const mine=m.sender_id===user.id;if(m.deleted_at)return '<div class="mm-bubble '+(mine?'mine':'theirs')+' deleted" data-message-id="'+esc(m.id)+'"><div>🚫 تم حذف هذه الرسالة</div><small>'+time(m.created_at)+'</small></div>';let content='';const type=m.message_type||'text';if(type==='image'&&m.media_url)content='<img class="mm-media-img" src="'+esc(m.media_url)+'" alt="صورة">';else if(type==='video'&&m.media_url)content='<video class="mm-media-video" controls playsinline preload="metadata" src="'+esc(m.media_url)+'"></video>';else if(type==='gif'&&m.media_url)content='<img class="mm-media-img" src="'+esc(m.media_url)+'" alt="GIF">';else if(type==='audio'&&m.media_url)content='<audio class="mm-audio" controls preload="metadata" src="'+esc(m.media_url)+'"></audio>';else if(type==='file'&&m.media_url)content='<a class="mm-file" href="'+esc(m.media_url)+'" target="_blank" rel="noopener">📎 <span>'+esc(m.file_name||'ملف')+'</span></a>';else content='<div>'+esc(m.body||'')+'</div>';if(m.reply_to_id)content='<div class="mm-reply-preview" data-reply-id="'+esc(m.reply_to_id)+'">↩️ <span>رد على رسالة</span></div>'+content;if(m.shared_from_id)content='<div class="mm-shared-label">↗️ رسالة مشتركة</div>'+content;if(m.body&&type!=='text')content+='<div class="mm-media-caption">'+esc(m.body)+'</div>';return '<div class="mm-bubble '+(mine?'mine':'theirs')+'" data-message-id="'+esc(m.id)+'">'+content+'<small>'+time(m.created_at)+(mine?'<span class="mm-read">'+(m.read_at?'✓✓':'✓')+'</span>':'')+'</small></div>'}
  function subscribe(){if(channel)sb.removeChannel(channel);channel=sb.channel('mada-chat-'+current.cid).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages',filter:'conversation_id=eq.'+current.cid},p=>{if(p.new.sender_id!==user.id){const out=$('mmMessages');out.querySelector('.chat-empty')?.remove();out.insertAdjacentHTML('beforeend',messageHtml(p.new));scrollBottom();toast('وصلتك رسالة جديدة 💬');if(window.madaMessengerAdvanced?.afterMessage)window.madaMessengerAdvanced.afterMessage(p.new)}}).on('postgres_changes',{event:'UPDATE',schema:'public',table:'messages',filter:'conversation_id=eq.'+current.cid},p=>{const row=document.querySelector('[data-message-id="'+CSS.escape(p.new.id)+'"]');if(row&&p.new.deleted_at){row.outerHTML=messageHtml(p.new)}else if(row&&p.new.sender_id===user.id&&p.new.read_at){const x=row.querySelector('.mm-read');if(x)x.textContent='✓✓'}}).subscribe()}
  async function send(){if(!get()||!current)return;const input=$('mmInput'),body=input.value.trim();if(!body)return;const btn=$('mmSend');btn.disabled=true;try{const {data,error}=await sb.from('messages').insert({conversation_id:current.cid,sender_id:user.id,body,message_type:'text'}).select('id,conversation_id,sender_id,body,created_at,read_at,message_type,media_url,file_name,file_size,reply_to_id,deleted_at,shared_from_id').single();if(error)throw error;input.value='';$('mmMessages').insertAdjacentHTML('beforeend',messageHtml(data));scrollBottom()}catch(e){console.error(e);toast('تعذر إرسال الرسالة')}finally{btn.disabled=false}}
