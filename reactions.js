...
(function(){
  ...
  let openId=null;
  const picker=id=>`<div class="mada-reaction-picker" data-picker="${id}" role="menu">${Object.entries(REACTIONS).map(([k,v])=>`<button type="button" class="mada-reaction" data-reaction="${k}" title="${v.label}" aria-label="${v.label}">${v.emoji}</button>`).join('')}</div>`;
  ...
  async function saveReaction(postId,type){
    ...
    let r;
    if(existing.data){
      r=existing.data.reaction_type===type?await sb.from('post_likes').delete().eq('post_id',postId).eq('user_id',user.id):await sb.from('post_likes').update({reaction_type:type}).eq('post_id',postId).eq('user_id',user.id);
  ...
  function addPickers(){
    document.querySelectorAll('.post-actions .like[data-id]').forEach(btn=>{
      ...
      btn.dataset.reactionReady='1';let timer=null;
      const show=()=>{const id=btn.dataset.id;closeAll(id);if(!btn.parentElement.querySelector(`[data-picker="${id}"]`)){btn.insertAdjacentHTML('afterend',picker(id));openId=id}};
      btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();if(openId===btn.dataset.id)closeAll();else show()});
