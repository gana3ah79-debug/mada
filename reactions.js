...
(function(){
  ...
  // Neutralize the legacy app handler that used to call loadFeed() after liking.
  if(typeof window.toggleLike==='function'&&!window.__MADA_TOGGLELIKE_NO_REFRESH){window.__MADA_TOGGLELIKE_NO_REFRESH=true;window.toggleLike=async function(id,liked){const client=sb();const me=await currentUser(client);if(!client||!me)return;const r=liked?await client.from('post_likes').delete().eq('post_id',id).eq('user_id',me.id):await client.from('post_likes').insert({post_id:id,user_id:me.id});if(r.error)alert('تعذر حفظ الإعجاب: '+r.error.message)}}
