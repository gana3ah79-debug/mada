...
(function(){'use strict';
  ...
  async function apply(){
    ...
    if(r.error)return;
    const d=r.data||{};
    if(d.stories_visible===false){
      ['.stories','#storyRow','#addStoryBtn','#allStoriesBtn'].forEach(s=>document.querySelectorAll(s).forEach(hide));
    }
    if(d.reels_visible===false){
      ['#reelsBtn','[data-reels]','.reels','.reels-section'].forEach(s=>document.querySelectorAll(s).forEach(hide));
    }
  }
