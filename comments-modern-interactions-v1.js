/* Mada modern comments interactions v2 — works on any comments route + dynamic like list/owner badge. */
(function(){'use strict';
...
async function showLikes(id,button){
  ...
  if(r.error)return; const rows=r.data||[]; if(!rows.length)return;
  ...
  const pop=document.createElement('div');pop.className='comment-likes-popover';
  ...
  if(window.innerWidth>600){const r=button.getBoundingClientRect();const w=Math.min(310,window.innerWidth-28);let left=r.left;left=Math.max(14,Math.min(left,window.innerWidth-w-14));pop.style.width=w+'px';pop.style.left=left+'px';pop.style.top=Math.max(12,r.top-pop.offsetHeight-8)+'px'}
}
...
function arrange(){document.querySelectorAll('.comment-actions').forEach(a=>{if(a.dataset.modernized)return;const like=a.querySelector('[data-like-comment]');if(!like)return;a.dataset.modernized='1';const buttons=[...a.children];const left=document.createElement('div'),right=document.createElement('div');left.className='comment-action-group';right.className='comment-action-group';buttons.forEach(b=>{(b===like?left:right).appendChild(b)});a.append(left,right)})}
