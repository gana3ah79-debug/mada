/* Mada Profile Post Social v2 — live likes, comments and shares without profile reload. */
(function(){'use strict';
if(window.__MADA_PROFILE_POST_SOCIAL_V2)return;window.__MADA_PROFILE_POST_SOCIAL_V2=true;
...
async function stats(article){
 ...
 const lc=likes.count||0,cc=comments.count||0,sc=shares.count||0;
 state.set(id,{liked});
 const box=article.querySelector('.mada-profile-post-actions');
 const summary=article.querySelector('.mada-profile-count-summary');
 if(summary){summary.querySelector('[data-summary="likes"]')?.replaceChildren(String(lc));summary.querySelector('[data-summary="comments"]')?.replaceChildren(String(cc));summary.querySelector('[data-summary="shares"]')?.replaceChildren(String(sc))}
 if(box){
  box.querySelector('[data-action="like"]')?.classList.toggle('active',liked);
  const a=box.querySelector('[data-action="like"] .count');if(a)a.textContent=lc;
  const b=box.querySelector('[data-action="comment"] .count');if(b)b.textContent=cc;
  const d=box.querySelector('[data-action="share"] .count');if(d)d.textContent=sc;
 }
}
...
const mo=new MutationObserver(scan);if(document.body)mo.observe(document.body,{childList:true,subtree:true});
scan();window.MadaProfilePostSocialV2={refresh:scan};
