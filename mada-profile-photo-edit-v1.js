/* Mada profile photo controls v2 */
(function(){
  'use strict';
  const STYLE_ID='mada-profile-photo-edit-style-v2';
  const CSS=`
    #modal .profile-photo-edit{position:absolute;display:flex;align-items:center;justify-content:center;width:36px;height:36px;border:2px solid #fff;border-radius:50%;background:#1877f2;color:#fff;box-shadow:0 3px 10px rgba(0,0,0,.22);cursor:pointer;z-index:20;font-size:17px;padding:0;line-height:1}
    #modal .profile-avatar-wrap{position:relative;width:max-content;margin:-48px auto 0;z-index:10}
    #modal .profile-avatar-wrap .profile-avatar{margin:0}
    #modal .profile-avatar-wrap .profile-photo-edit{right:-3px;bottom:2px}
  ...
  function decorate(root){
    if(!root||root.dataset.photoEditBound==='2')return;
    const edit=document.getElementById('editProfile');
    if(!edit)return;
    root.dataset.photoEditBound='2';
    ...
    if(cover&&!cover.parentElement.classList.contains('profile-cover-wrap')){
      ...
      const b=document.createElement('button');b.type='button';b.className='profile-photo-edit';b.title='تعديل صورة الغلاف';b.setAttribute('aria-label','تعديل صورة الغلاف');b.textContent='📷';b.addEventListener('click',openEditor);wrap.appendChild(b);
    }
    ...
    if(avatar&&!avatar.parentElement.classList.contains('profile-avatar-wrap')){
      ...
      const b=document.createElement('button');b.type='button';b.className='profile-photo-edit';b.title='تعديل صورة الملف الشخصي';b.setAttribute('aria-label','تعديل صورة الملف الشخصي');b.textContent='📷';b.addEventListener('click',openEditor);wrap.appendChild(b);
    }
  }
  function decorateEdit(){
    const a=document.getElementById('editAvatar'),c=document.getElementById('editCover');
    [a,c].forEach(input=>{if(!input||input.dataset.photoPreview==='2')return;input.dataset.photoPreview='2';const img=document.createElement('img');img.className='edit-photo-preview';input.parentElement.appendChild(img);input.addEventListener('change',()=>{const f=input.files&&input.files[0];if(!f){img.style.display='none';return}if(img.dataset.url)URL.revokeObjectURL(img.dataset.url);img.dataset.url=URL.createObjectURL(f);img.src=img.dataset.url;img.style.display='block'})});
  }
