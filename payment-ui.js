...
(function(){
 ...
 async function settings(){const {data,error}=await sb.from('admin_settings').select('cash_wallet,subscription_price_egp').eq('id',true).maybeSingle();if(error)console.warn(error);return data||{cash_wallet:null,subscription_price_egp:99}}
 async function myRequest(){const {data:{user}}=await sb.auth.getUser();if(!user)return null;const {data}=await sb.from('subscription_requests').select('id,transaction_ref,receipt_image_url,status,created_at').eq('user_id',user.id).order('created_at',{ascending:false}).limit(1).maybeSingle();return data||null}
 async function open(){
  const {data:{user}}=await sb.auth.getUser();if(!user)return alert('سجّل الدخول أولاً.');
 ...
 async function submit(){
  ...
  try{
   ...
   const {error}=await sb.from('subscription_requests').insert({user_id:user.id,transaction_ref:ref,receipt_image_url:path,status:'pending'});
