/* Mada authentication: single isolated controller. */
(function(){
  ...
  const getClient=()=>{
    try{
      ...
      if(!window.supabase?.createClient||!window.MADA_SUPABASE_URL||!window.MADA_SUPABASE_KEY)return null;
      return window.__madaAuthClient=window.supabase.createClient(window.MADA_SUPABASE_URL,window.MADA_SUPABASE_KEY);
    }catch(e){console.error('Mada auth client',e);return null;}
  };
  function render(){
    ...
    if(loginBtn){loginBtn.innerHTML=signup?'<span>إنشاء الحساب</span><span class="arrow">✨</span>':'<span>تسجيل الدخول</span><span class="arrow">←</span>';loginBtn.disabled=busy;loginBtn.style.pointerEvents='auto';loginBtn.style.touchAction='manipulation';loginBtn.style.position='relative';loginBtn.style.zIndex='2147483647';}
    if(signupBtn){signupBtn.textContent=signup?'العودة لتسجيل الدخول':'إنشاء حساب جديد ✨';signupBtn.disabled=busy;signupBtn.style.pointerEvents='auto';signupBtn.style.touchAction='manipulation';signupBtn.style.position='relative';signupBtn.style.zIndex='2147483647';}
  ...
  async function finish(c,u){
    ...
    try{if(typeof window.loadProfile==='function')await window.loadProfile()}catch(e){console.warn('Mada profile after login',e)}
    try{if(typeof window.loadFeed==='function')await window.loadFeed()}catch(e){console.warn('Mada feed after login',e)}
    try{if(typeof window.madaRefreshSocialBadges==='function')await window.madaRefreshSocialBadges()}catch(e){}
  }
  const withTimeout=(promise,ms)=>Promise.race([promise,new Promise((_,reject)=>setTimeout(()=>reject(new Error('انتهت مهلة الاتصال بالخادم')),ms))]);
  async function login(){
    if(busy)return;
    const email=($('emailInput')?.value||'').trim(),password=$('passwordInput')?.value||'';
    if(!email||!password){message('يرجى إدخال البريد الإلكتروني وكلمة المرور',true);return;}
    const c=getClient();if(!c){message('خدمة الحسابات غير جاهزة. حدّث الصفحة وحاول مرة أخرى.',true);return;}
    busy=true;render();message('جارٍ تسجيل الدخول…');
    try{
      const r=await withTimeout(c.auth.signInWithPassword({email,password}),15000);
      if(r.error)throw r.error;
      if(!r.data?.session?.user)throw new Error('لم يتم إنشاء جلسة دخول');
      await finish(c,r.data.session.user);
  ...
  async function signup(){
    ...
    try{
      const r=await withTimeout(c.auth.signUp({email,password,options:{data:{display_name:name,full_name:name}}}),15000);
      if(r.error)throw r.error;
      if(r.data?.session?.user)await finish(c,r.data.session.user);else message('تم إنشاء الحساب. افتح بريدك واضغط رابط التأكيد، ثم سجّل الدخول.');
  ...
  async function reset(){
    ...
    const c=getClient();if(!c){message('خدمة الحسابات غير جاهزة. حدّث الصفحة وحاول مرة أخرى.',true);return;}
    try{const r=await withTimeout(c.auth.resetPasswordForEmail(email,{redirectTo:location.origin+'/reset-password.html'}),15000);if(r.error)throw r.error;message('تم إرسال رابط استعادة كلمة المرور إلى بريدك الإلكتروني ✓');}catch(e){console.error('Mada reset',e);message('تعذر إرسال رابط الاستعادة: '+(e?.message||'حاول مرة أخرى'),true);}
  }
  function handle(e){
    ...
    if(t.id==='signupBtn'){if(!busy){mode=mode==='login'?'signup':'login';render();}return;}
    reset();
  }
  function capture(e){
    const t=e.target?.closest?.('#loginBtn,#signupBtn,#forgotPasswordBtn');if(!t)return;
    e.preventDefault();e.stopImmediatePropagation();handle(e);
  }
  function bind(){
  ...
  /* Capture at document level so mobile touch/click quirks cannot bypass the auth action. */
  ...
  document.addEventListener('submit',e=>{
    const form=e.target;if(!form||form.id!=='madaAuthForm')return;
    e.preventDefault();mode==='login'?login():signup();
  },true);
