...
(function(){
  ...
  async function login(){
    ...
    busy=true;const b=$('loginBtn'),old=b?.innerHTML;if(b){b.disabled=true;b.innerHTML='<span>جارٍ تسجيل الدخول…</span>'}message('جارٍ التحقق من بيانات الدخول…');
    try{const {data,error}=await c.auth.signInWithPassword({email,password});if(error)throw error;if(!data?.session?.user)throw new Error('لم يتم إنشاء جلسة دخول');await finish(c,data.session.user);message('تم تسجيل الدخول بنجاح ✓')}
  ...
  async function signup(){
    ...
    const c=client();if(!c){message('خدمة الحسابات غير جاهزة. حدّث الصفحة وحاول مرة أخرى.',true);return}
    busy=true;const b=$('loginBtn'),old=b?.innerHTML;if(b){b.disabled=true;b.innerHTML='<span>جارٍ إنشاء الحساب…</span>'}message('جارٍ إنشاء الحساب…');
    try{const {data,error}=await c.auth.signUp({email,password,options:{data:{display_name:name,full_name:name}}});if(error)throw error;if(data?.session?.user){await finish(c,data.session.user);message('تم إنشاء الحساب وتسجيل الدخول ✓')}else{message('تم إنشاء الحساب. افتح بريدك واضغط رابط التأكيد، ثم سجّل الدخول.')}}
