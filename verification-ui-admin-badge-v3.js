...
function render(el,p){if(!el||p?.role!=='admin'||p?.is_banned)return;el.querySelectorAll(':scope > .mada-verified-badge.admin').forEach(x=>x.remove());el.insertAdjacentHTML('beforeend',BADGE);el.classList.add('mada-admin-verified-target')}
...
let timer=0;const queue=()=>{if(timer)return;timer=setTimeout(()=>{timer=0;decorate(document)},80)};
