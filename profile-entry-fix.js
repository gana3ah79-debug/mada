(function(){
  ...
  function fixPostMenus(){
    const current=window.user||(typeof user!=='undefined'?user:null);
    if(!current)return;
    document.querySelectorAll('#feed article.post[id^="post-"]').forEach(article=>{
      const authorId=article.dataset.authorId||article.querySelector('[data-profile]')?.dataset.profile||'';
      article.dataset.authorId=authorId;
      const menu=article.querySelector('.mada-post-menu');
      if(!menu)return;
      menu.style.background='#fff';
      menu.style.color='#27344a';
      menu.style.borderColor='#e3e8ef';
      ...
      if(authorId===current.id&&!menu.querySelector('[data-post-action="delete"]')){
        const del=document.createElement('button');
        del.type='button';del.className='danger';del.dataset.postAction='delete';del.textContent='🗑️ حذف المنشور';
  ...
  function bind(){
    const menu=$('menuBtn');
    if(menu&&!menu.dataset.profileEntryFix){
    ...
    if(feed&&!feed.dataset.postMenuFixBound){
      feed.dataset.postMenuFixBound='1';
      const observer=new MutationObserver(()=>setTimeout(fixPostMenus,0));
      observer.observe(feed,{childList:true,subtree:true});
      feed.addEventListener('click',()=>setTimeout(fixPostMenus,0),true);
    }
  }
