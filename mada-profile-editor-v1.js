/* Mada Profile Editor v1 — reliable name, username, bio and city editing. */
(function(){'use strict';
if(window.__MADA_PROFILE_EDITOR_V1)return;window.__MADA_PROFILE_EDITOR_V1=true;
...
function install(){
 document.addEventListener('click',async e=>{
  ...
  const p=window.__MADA_PROFILE_DATA,u=await C().auth.getUser();
  if(!p||!u.data?.user||p.id!==u.data.user.id)return;
  e.preventDefault();e.stopImmediatePropagation();
  show('✏️ تعديل الملف الشخصي',`<div class="mada-editor-v1">
  ...
  s.onclick=async()=>{
   ...
   try{
    ...
    const r=await C().from('profiles').update(payload).eq('id',p.id).select('id,display_name,username,bio,avatar_url,cover_url,city,role,is_verified,verification_type,verification_expires_at,created_at,updated_at').single();
...
const style=document.createElement('style');style.textContent=`.mada-editor-v1{display:grid;gap:12px;padding:4px}.mada-editor-v1 label{display:grid;gap:6px;font-weight:800;font-size:13px}.mada-editor-v1 input,.mada-editor-v1 textarea{width:100%;box-sizing:border-box;border:1px solid #d9dce1;border-radius:10px;padding:11px;font:inherit;background:#fff}.mada-editor-v1 textarea{min-height:100px;resize:vertical}.mada-editor-v1 button{min-height:44px;border:0;border-radius:10px;cursor:pointer;font:inherit;font-weight:900}.mada-editor-v1 .primary{background:#1877f2;color:#fff}`;document.head.appendChild(style);install();
})();