(()=>{
 ...
 function cleanup(page){
  keepOne(page,'.fb-profile-top');
  keepOne(page,'.fb-details');
  keepOne(page,'.fb-friends');
  keepOne(page,'.fb-profile-composer');
  keepOne(page,'.fb-section-title');
 }
 function enhance(){
  const page=document.querySelector('#modal .profile-page'); if(!page)return;
  const id=window.__MADA_PROFILE_ID; if(!id)return;
  cleanup(page);
  if(page.dataset.fbEnhanced===id){return}
  page.dataset.fbEnhanced=id; lastId=id;
 ...
 async function loadDetails(id,box){
  try{const r=await sb().from('profiles').select('*').eq('id',id).maybeSingle();const p=r.data||{};const list=[];
   if(p.location)list.push('📍 '+esc(p.location)); else if(p.city)list.push('📍 '+esc(p.city));
   if(p.relationship_status)list.push('💗 '+esc(p.relationship_status));
   if(p.work)list.push('💼 '+esc(p.work)); if(p.education)list.push('🎓 '+esc(p.education));
   ...
   if(box.isConnected)box.querySelector('.fb-detail-list').innerHTML=list.map(x=>'<div><span>'+x+'</span></div>').join('');
  }catch(e){if(box.isConnected)box.querySelector('.fb-detail-list').innerHTML='<div><span>ℹ️ لا توجد تفاصيل إضافية مضافة بعد.</span></div>'}
 }
 async function loadFriends(id,page){
  try{
   const old=page.querySelector('.fb-friends');if(!old)return;
   const[a,b]=await Promise.all([sb().from('friendships').select('addressee_id').eq('requester_id',id).eq('status','accepted'),sb().from('friendships').select('requester_id').eq('addressee_id',id).eq('status','accepted')]);
   if(!old.isConnected)return;
   const ids=[...new Set([...(a.data||[]).map(x=>x.addressee_id),...(b.data||[]).map(x=>x.requester_id)])].filter(x=>x&&x!==id).slice(0,6);
   const grid=document.createElement('div');grid.className='fb-friends-grid';
   old.innerHTML='<div class="fb-section-head"><h3>الأصدقاء</h3><button type="button">عرض الكل</button></div>';old.appendChild(grid);
   if(ids.length){const r=await sb().from('profiles').select('id,display_name,avatar_url').in('id',ids);if(!old.isConnected)return;grid.innerHTML=(r.data||[]).map(p=>'<button type="button" data-friend="'+p.id+'"><span class="fb-friend-avatar">'+(p.avatar_url?'<img src="'+esc(p.avatar_url)+'">':ini(p.display_name))+'</span><b>'+esc(p.display_name||'مستخدم')+'</b></button>').join('')}
   else grid.innerHTML='<div class="empty">لا يوجد أصدقاء حتى الآن.</div>';
   old.querySelector('.fb-section-head button').onclick=()=>page.querySelector('#profileFriendsCount')?.click();
   grid.querySelectorAll('[data-friend]').forEach(b=>b.onclick=()=>window.ProfileUI?.open?.(b.dataset.friend));
